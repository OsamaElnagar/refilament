...
/**
 * The named color palettes and the color-conversion/contrast helpers behind
 * them (mirrors Filament's `Support\Colors\Color`). Palettes are the Tailwind
 * v4 50→950 shades expressed in `oklch()`, and the static helpers convert
 * between `#hex` / `rgb()` / `oklch()` and answer WCAG contrast questions —
 * the same math the panel uses to pick an accessible text shade on a surface.
 */
class Color
{
    /**
     ...
     */
    public const float WCAG_AA_TEXT = 4.5;

    /**
     * WCAG 2.1 minimum contrast ratio for large-scale text at conformance level AA
     * (success criterion 1.4.3 Contrast (Minimum)).
     */
    public const float WCAG_AA_LARGE_TEXT = 3.0;
