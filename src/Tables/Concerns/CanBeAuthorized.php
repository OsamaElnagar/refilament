...
namespace Refilament\Refilament\Tables\Concerns;
...
trait CanBeAuthorized
{
     ...
     *
     * @var array{type: 'all'|'any', abilities: array<int, string>, arguments: array<int, mixed>}|Closure(object): mixed|null
     */
     ...
     *
     * @param  UnitEnum|BackedEnum|string|array<int, UnitEnum|BackedEnum|string>|Closure(object): mixed  $abilities
    ...
    /**
     * @return array{type: 'all'|'any', abilities: array<int, string>, arguments: array<int, mixed>}|Closure(object): mixed|null
     */
    ...
    protected function resolveAuthorizationResponse(array $prependedArguments): AuthResponse
    {
        ...
        if ($this->authorization instanceof Closure) {
            // A record-less closure authorization (header/bulk actions) is
            // called with no arguments, but the declared signature may type the
            // record for row actions — PHPStan cannot prove the zero-arg call.
            if ($prependedArguments === []) {
                // @phpstan-ignore arguments.count (record-less closure authorizations take no record)
                $resolved = ($this->authorization)();
            } else {
                $resolved = ($this->authorization)($prependedArguments[0]);
            }
